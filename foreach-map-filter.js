...
function find(arr, searchValue) {
    let filteredArray = arr.filter(element => element === searchValue);
...
function findInObj(arr, key, searchValue) {
    let filtered = arr.filter(obj => obj[key] === searchValue);
...
function doubleOddNumbers(arr) {
    const oddOnly = arr.filter(value => value % 2 != 0);
    return oddOnly.map(value => value*2); 
}